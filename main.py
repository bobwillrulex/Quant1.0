...
def split_csv_values(raw_value: str, *, uppercase: bool = False) -> List[str]:
    values: List[str] = []
    for part in raw_value.split(","):
        token = part.strip()
        if not token:
            continue
        values.append(token.upper() if uppercase else token)
    return values
        ...
        if request.method == "POST":
            try:
                if mode == "present":
                    ...
                    ticker_values = split_csv_values(ticker, uppercase=True)
                    model_name_values = [sanitize_model_name(v) for v in split_csv_values(model_name)]
                    if (
                        train_action == "train"
                        and selected_model == "__new__"
                        and len(ticker_values) > 1
                    ):
                        if not model_name_values:
                            raise ValueError("Provide comma-separated model names when training multiple tickers.")
                        if len(ticker_values) != len(model_name_values):
                            raise ValueError("Ticker count must match model name count for batch training.")
                        if len(set(model_name_values)) != len(model_name_values):
                            raise ValueError("Model names in batch training must be unique.")
                        existing_names = set(saved_models)
                        for candidate in model_name_values:
                            if not candidate:
                                raise ValueError("Model names cannot be empty in batch training.")
                            if candidate in existing_names:
                                raise ValueError(f"Model already exists: {candidate}")
                        model_configs = load_model_configs(mode_key)
                        batch_rows = []
                        for batch_ticker, batch_model_name in zip(ticker_values, model_name_values):
                            dataset = fetch_yahoo_rows(ticker=batch_ticker, interval=interval, row_count=row_count)
                            bundle = train_strategy_models(dataset, split_style=split_style)
                            save_model_bundle(mode_key, batch_model_name, bundle)
                            model_configs[batch_model_name] = {
                                "ticker": batch_ticker,
                                "interval": interval,
                                "rows": row_count,
                                "include_in_run_all": True,
                            ...
                            batch_rows.append(
                                "<tr>"
                                f"<td>{batch_model_name}</td>"
                                f"<td>{batch_ticker}</td>"
                                f"<td>{int(bundle['train_size'])}</td>"
                                f"<td>{int(bundle['test_size'])}</td>"
                                "</tr>"
                            )
                        save_model_configs(mode_key, model_configs)
                        saved_models = list_saved_models(mode_key)
                ...
                <section class="results">
                  <div class="section-heading">
                    <h2>Batch Training Complete</h2>
                    <p class="muted">Interval: {interval} | Rows: {row_count} | Split: {split_style} | BUY&gt;{buy_threshold:.2f} / SELL&lt;{sell_threshold:.2f}</p>
                  </div>
                  <article class="card table-card">
                    <h3>Saved Models</h3>
                    <table>
                      <tr><th>Model</th><th>Ticker</th><th>Train Rows</th><th>Test Rows</th></tr>
                      {"".join(batch_rows)}
                    </table>
                  </article>
                </section>
        ...
        <html>
          ...
          <body>
            ...
            <form method="post" class="card">
              ...
              <div class="form-grid">
              <label>Ticker (comma-separated for batch):
                <input type="text" name="ticker" value="{ticker}" placeholder="MSFT,AMZN" required />
              </label>
              <label>Interval:
              ...
              <label>New Model Name (optional, comma-separated for batch):
                <input type="text" name="model_name" value="{model_name}" placeholder="microsoft,amazon" />
              </label>
